...
/*   Updated: 2024/05/07 14:17:00 by mmad             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_if_plus_eql(char *str) // can not cause leaks
{
	int i;
...
int	ft_strchr_edit(const char *s, int c) // same function exit
{
	size_t i;
...
void	ft_print_list_helper(t_list *head)
{
	while (head)
	{
		printf("[%s] [%s] [%s] \n", (char *)head->content, (char *)head->key,
			(char *)head->value);
		head = head->next;
	}
	// printf("-----------------------------------------\n");
